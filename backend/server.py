    ...
    def find_by_username_or_email(self, credential: str) -> dict | None:
        ...
        with self._lock:
            for user in self._users["users"]:
                if (
                    user["username"].lower() == needle
                    or user["email"].lower() == needle
                ):
                    return user
        return None
    ...
    def create_user(self, username: str, email: str, password: str) -> tuple[bool, dict | str]:
        ...
        with self._lock:
            for existing in self._users["users"]:
                if (
                    existing["username"].lower() == user_record["username"].lower()
                    or existing["email"].lower() == user_record["email"].lower()
                ):
...
class WorldStore:
    ...
    def upsert_player(self, user_id: str, username: str, state: dict) -> None:
        now = int(time.time())
        position = state.get("position", {"x": 196.0, "y": 143.0})
        velocity = state.get("velocity", {"x": 0.0, "y": 0.0})
        last_dir = state.get("last_dir", {"x": 0.0, "y": 1.0})
        with self._lock:
            self._players[user_id] = {
                ...
                "velocity": velocity,
                "last_dir": last_dir,
                "attack_pressed": bool(state.get("attack_pressed", False)),
                "updated_at": now,
            }
                ...
                players.append(
                    {
                        ...
                        "attack_pressed": player["attack_pressed"],
                    }
                )
    ...
    def _prune_locked(self, now: int) -> None:
        ...
        for user_id, player in self._players.items():
            if now - int(player.get("updated_at", 0)) > PLAYER_TIMEOUT_SECONDS:
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _require_user(self) -> tuple[dict | None, str | None]:
    ...
    def _handle_register(self) -> None:
        ...
        cookie = (
            f"session_id={sid}; HttpOnly; SameSite=Lax; Path=/; Max-Age={SESSION_TTL_SECONDS}"
        )
        ok(
            self,
            {
    ...
    def _handle_login(self) -> None:
        ...
        if not user or not _verify_password(password, user["password_hash"]):
            err(
                ...
                "INVALID_CREDENTIALS",
                "Credential or password is incorrect",
            )
            return
    ...
    def _handle_state_update(self) -> None:
        ...
        if not isinstance(player_state, dict):
            err(self, HTTPStatus.BAD_REQUEST, "VALIDATION_ERROR", "player_state must be an object")
            return

        self.world_store.upsert_player(user["id"], user["username"], player_state)
        ok(self, self.world_store.snapshot())
...
def run(host: str, port: int) -> None:
    ...
    Handler.world_store = WorldStore()
