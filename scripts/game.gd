...
func _start_gameplay(data: Dictionary) -> void:
	...
	local_player_id = str(profile.get("id", ""))
	if local_player_id.is_empty():
		_set_status("Missing profile id")
		return

	# Local enemy remains deterministic in every client.
	spawn_from_state({
		"entities": [
			{
				"id": "enemy_1",
	...
	_set_status("Connected as %s" % profile.get("username", "player"))
	_sync_world_loop()


func _sync_world_loop() -> void:
	while true:
		var local_player := players.get(local_player_id)
		if local_player != null:
			var update_result := await api_client.push_state(local_player.get_net_state())
			if update_result.get("ok", false):
				_apply_world_snapshot(update_result["data"])
			else:
				_show_error(update_result)
		else:
			_spawn_or_update_player({
				"id": local_player_id,
				"is_local": true,
...
func _apply_world_snapshot(world_data: Dictionary) -> void:
	var seen_ids: Dictionary = {}
	for player_snapshot in world_data.get("players", []):
		var id := str(player_snapshot.get("id", ""))
		if id.is_empty():
			continue
		seen_ids[id] = true
		player_snapshot["is_local"] = id == local_player_id
	...
	for existing_id in players.keys():
		if existing_id == local_player_id:
			continue
		if not seen_ids.has(existing_id):
			var stale = players[existing_id]
			if stale:
				stale.queue_free()
			players.erase(existing_id)
